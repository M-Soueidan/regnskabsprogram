              ...
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={() => void updatePlan(plan, { active: !plan.active })}
                    className={
                      'inline-flex items-center gap-1.5 text-xs font-medium text-slate-700 ' +
                      (plan.active ? '' : 'opacity-70')
                    }
                    title={plan.active ? 'Aktiv — klik for at deaktivere' : 'Deaktiveret — klik for at aktivere'}
                  >
                    <span
                      className={
                        'relative inline-flex h-5 w-9 shrink-0 items-center rounded-full transition-colors ' +
                        (plan.active ? 'bg-indigo-600' : 'bg-slate-300')
                      }
                    >
                      <span
                        className={
                          'inline-block h-4 w-4 rounded-full bg-white shadow transition-transform ' +
                          (plan.active ? 'translate-x-[18px]' : 'translate-x-0.5')
                        }
                      />
                    </span>
                    Aktiv
